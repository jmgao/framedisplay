...
int MBAACC_CG::get_image_count() { return m_nimages; }

void MBAACC_CG::copy_cells(const MBAACC_CG_Image* image, const MBAACC_CG_Alignment* align,
                           unsigned char* pixels, unsigned int x1, unsigned int y1,
                           unsigned int width, unsigned int height, unsigned int* palette,
                           bool is_8bpp) {
...
Texture* MBAACC_CG::draw_texture(unsigned int n, unsigned int* palette, bool to_pow2_flg,
                                 bool draw_8bpp) {
  const MBAACC_CG_Image* image = get_image(n);
  if (!image) {
    return 0;
