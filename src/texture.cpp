...
bool Texture::save_to_png(const char* filename, unsigned int* palette) {
  ...
  png_set_IHDR(png_write, png_info, m_width, m_height, 8,
               m_8bpp ? PNG_COLOR_TYPE_PALETTE : PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE,
...
void Texture::blend_mode(int mode) { m_blend_mode = mode; }
...
void Texture::scale_clear() { m_scale = 0; }
...
void Texture::alpha(float alpha) { m_alpha = alpha; }

void Texture::special_mode(int mode) { m_mode = mode; }
