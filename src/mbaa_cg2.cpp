...
int MBAA_CG2::get_image_count() { return m_nimages; }

Texture* MBAA_CG2::draw_texture(unsigned int n, MBAA_ARX* arx, unsigned int* palette,
                                bool to_pow2_flg, bool draw_8bpp) {
  if (!arx) {
    return 0;
  }
  ...
  for (unsigned int i = 0; i < image->align_len; ++i) {
    ...
    tile_image->copy_region_to(pixels, align->x - x1, align->y - y1, width, height, align->source_x,
                               align->source_y, align->width, align->height, palette, is_8bpp);
    ++align;
  }
