...
void MBAACC_Character::set_render_properties(const MBAACC_Frame* frame, Texture* texture) {
  ...
  if (frame->AF.blend_mode != 0) {
    texture->color((float)frame->AF.red / 255.0f, (float)frame->AF.green / 255.0f,
...
Clone* MBAACC_Character::make_clone(int seq_id, int fr_id) {
  ...
  if (frame->AF.active && frame->AF.frame >= 0) {
    ...
    if (texture) {
      ...
      clone->init_texture(texture, (frame->AF.offset_x - 128) * 2, (frame->AF.offset_y - 224) * 2,
                          2);
    }
  }
...
void MBAACC_Character::render_frame_properties(bool detailed, int scr_width, int scr_height,
                                               int seq_id, int fr_id) {
  if (!m_loaded) {
    return;
  }
  ...
  y += render_shaded_text(x, y, "$yState   :$. %s",
                          frame->AS->stand_state == 0
                              ? "Standing"
                              : frame->AS->stand_state == 1
                                    ? "Airborne"
  ...
  frame->AF.jump_to_seq);
  } else if (frame->AS->move_to_frame >= 0) {
          y += render_shaded_text(x, y, "$y-$. Go to state %d",
  frame->AS->move_to_frame + 1);
  } else {
  ...
  if (frame->AT) {
    y += render_shaded_text(
        ...
        "$yProration:$. %d%%\n"
        "$yCircuit +:$. %.1f%%",
        frame->AT->guard_flags & 0x400 ? " - " : frame->AT->guard_flags & 4 ? " Y " : " N ",
        frame->AT->guard_flags & 0x100 ? " - " : frame->AT->guard_flags & 1 ? " Y " : " N ",
        frame->AT->guard_flags & 0x200 ? " - " : frame->AT->guard_flags & 2 ? " Y " : " N ",
        frame->AT->damage, frame->AT->red_damage,
        frame->AT->proration != 0 ? frame->AT->proration : 100, frame->AT->circuit_gain / 100.0f);
...
void MBAACC_Character::free_graphics() { m_cg.free(); }
...
MBAACC_Character::~MBAACC_Character() { free(); }
