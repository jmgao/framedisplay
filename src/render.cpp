...
void render_font_init() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA8, 256, 64, 0, GL_ALPHA, GL_UNSIGNED_BYTE, data);

  has_font = 1;
}

void render_font_clear() { glDeleteTextures(1, &font_tex_id); }
  ...
  do {
    ...
    while (cur->next != NULL) {
      ...
      do {
        ...
        if ((cur->x1 >= tmp->x1) && (cur->x2 <= tmp->x2) && (cur->y1 >= tmp->y1) &&
            (cur->y2 <= tmp->y2)) {
          node* tmp2 = first;
          if (cur == first) {
            first = first->next;
