...
static const unsigned int table[24] = {
	0x1,		0x3,		0x7,		0xf,
	0x1f,		0x3f,		0x7f,		0xff,
...
bool Ougon_Data::decompress(unsigned char *data, unsigned int size, unsigned char **ddest, unsigned int *dsize) {
	unsigned int v1;
	unsigned int v2;
	unsigned char *input;
...
bool Ougon_Data::load_and_decomp(const char *filename, const char *base_path,
...
Texture *Ougon_Data::get_sprite(int id) {
	...
	if (!memcmp(tex_data, "DDS ", 4)
	    && tex_size > 0x100
	    && ((unsigned int *)tex_data)[0x04/4] == 0x7c
	    && ((unsigned int *)tex_data)[0x4c/4] == 0x20) {
		struct header_1_t {
			unsigned int	flags;
			unsigned int	height;
			unsigned int	width;
