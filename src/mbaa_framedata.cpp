...
static unsigned int* fd_frame_load(unsigned int* data, const unsigned int* data_end,
                                   MBAA_Frame* frame, TempInfo* info) {
  memset(frame, 0, sizeof(MBAA_Frame));
  ...
  while (data < data_end) {
    ...
    if (!memcmp(buf, "HRNM", 4) || !memcmp(buf, "HRAT", 4)) {
      ...
      if (n <= 32 && info->cur_hitbox < info->seq->nhitboxes) {
        MBAA_Hitbox* d = &info->seq->hitboxes[info->cur_hitbox];
        ++info->cur_hitbox;
  ...
  while (data < data_end) {
    ...
    if (!memcmp(buf, "PSTS", 4)) {
      ...
      if (data[0] == 32) {
        ...
        int size;
        size = sizeof(MBAA_Frame) * seq->nframes;
        size += sizeof(MBAA_Hitbox) * seq->nhitboxes;
        size += sizeof(MBAA_Frame_AT) * seq->nAT;
        size += sizeof(MBAA_Frame_AS) * seq->nAS;
        size += sizeof(MBAA_Frame_EF) * seq->nEF;
        size += sizeof(MBAA_Frame_IF) * seq->nIF;

        char* data = new char[size];
        seq->data = data;

        seq->frames = (MBAA_Frame*)data;
        size = sizeof(MBAA_Frame) * seq->nframes;

        seq->hitboxes = (MBAA_Hitbox*)(data + size);
        size += sizeof(MBAA_Hitbox) * seq->nhitboxes;

        seq->AT = (MBAA_Frame_AT*)(data + size);
        size += sizeof(MBAA_Frame_AT) * seq->nAT;

        seq->AS = (MBAA_Frame_AS*)(data + size);
        size += sizeof(MBAA_Frame_AS) * seq->nAS;

        seq->EF = (MBAA_Frame_EF*)(data + size);
        size += sizeof(MBAA_Frame_EF) * seq->nEF;

        seq->IF = (MBAA_Frame_IF*)(data + size);
      ...
      if (seq->initialized && frame_no < seq->nframes) {
        MBAA_Frame* frame = &seq->frames[frame_no];
...
static unsigned int* fd_main_load(unsigned int* data, const unsigned int* data_end,
                                  MBAA_Sequence** sequences, unsigned int nsequences) {
  while (data < data_end) {
    unsigned int* buf = data;
    ++data;
    ...
    if (!memcmp(buf, "PSTR", 4)) {
      ...
      if (memcmp(data, "PEND", 4)) {
        if (seq_id < nsequences) {
          MBAA_Sequence* seq = sequences[seq_id];
          if (!seq) {
            seq = new MBAA_Sequence;
...
bool MBAA_FrameData::load(MBAA_ISO* iso, const char* filename) {
  ...
  if (m_sequences) {
    if (sequence_count > m_nsequences) {
      MBAA_Sequence** seqs = new MBAA_Sequence*[sequence_count];
    ...
    m_nsequences = sequence_count;
    m_sequences = new MBAA_Sequence*[m_nsequences];
    for (unsigned int i = 0; i < m_nsequences; ++i) {
      m_sequences[i] = 0;
    }
  }
...
bool MBAA_FrameData::load_move_list(MBAA_ISO* iso, const char* filename) {
  if (!m_loaded) {
    return 0;
  }
...
void MBAA_FrameData::free() {
  if (m_sequences) {
    for (unsigned int i = 0; i < m_nsequences; ++i) {
      MBAA_Sequence* seq = m_sequences[i];
...
int MBAA_FrameData::get_sequence_count() {
  if (!m_loaded) {
    return 0;
  }
  return m_nsequences;
...
MBAA_Sequence* MBAA_FrameData::get_sequence(int n) {
  if (!m_loaded) {
    return 0;
  }
  ...
  MBAA_Sequence* seq = m_sequences[n];
...
MBAA_FrameData::~MBAA_FrameData() { free(); }
