...
bool Touhou_Imagedata::load_th105_from_framedata(Touhou_Packfiles *packfile, const char *charname, char *data, int size) {
	...
	for (int i = 0; i < 8; ++i) {
		sprintf(filename, "data/character/%s/palette%3.3d.pal",
			charname, i);
...
int Touhou_Imagedata::get_image_count() {
	return m_nimages;
}

int Touhou_Imagedata::get_palette_count() {
	return m_npalettes;
}
