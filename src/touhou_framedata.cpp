...
static bool read_boxes(char *&data, unsigned int &n, int *nboxes, Touhou_Rect **boxes, bool is_int) {
	if (is_int) {
		if (n < 4) {
			return 0;
		}
...
static bool read_attack_boxes(char *&data, unsigned int &n, int *nboxes, Touhou_Rect **boxes, bool is_int, int atkboxsize) {
	int max;
	if (is_int) {
		if (n < 4) {
			return 0;
...
bool Touhou_Framedata::init_th105_123(Touhou_Packfiles *pack, Touhou_Imagedata *imagedata, const char *name) {
 	if (m_initialized) {
		return 0;
	}
...
unsigned int Touhou_Framedata::get_sequence_count() {
	return m_nsequences;
}

bool Touhou_Framedata::has_sequence(unsigned int n) {
	return (n >= 0 && n < m_nsequences);
}
